...
// Set the base hook folder based on this file
define('HOOK_PATH_ROOT', dirname(__FILE__).DS);
...
function get_hook_include_files($function, $project_id = null) {
	...
	$script = dirname(__FILE__).DS."global".DS."global_hooks.php";
	if (file_exists($script)) $paths[] = $script;
	
	// GLOBAL HOOKS PER-FILE
	$script = dirname(__FILE__).DS."global".DS.$function.".php";
	if (file_exists($script)) $paths[] = $script;
	
	// PROJECT-SPECIFIED HOOKS IN ONE FILE
	$script = dirname(__FILE__).DS."pid".$project_id.DS."custom_hooks.php";
	if (file_exists($script)) $paths[] = $script;
	
	// PROJECT-SPECIFIC HOOKS PER-FILE (PREVIOUS VERSION)
	$script = dirname(__FILE__).DS."pid".$project_id.DS.$function.".php";
	if (file_exists($script)) $paths[] = $script;
